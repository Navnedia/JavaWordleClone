...
public class WordleWord implements Serializable {
	 ...
	 * @return A {@code int[]} with the evaluations of each index/letter of the word.
	...
	public int[] evaluate(String guess) {
		...
		int[] evaluations = new int[wordValue.length()];
	...
	public static String[] translateEvaluation(int[] numEvaluation) {
		String[] strEvaluation = new String[numEvaluation.length];
		
		for (int i = 0; i < strEvaluation.length; i++) {
			switch (numEvaluation[i]) {
	...
	public static void main(String[] args) {
		...
		int[] evaluation = answer.evaluate("title");
		
		int[] test = {1, 0, -1, 0, 3};
		
		System.out.println(Arrays.toString(translateEvaluation(evaluation)));
		
		System.out.println(Arrays.toString(translateEvaluation(test)));
	}
